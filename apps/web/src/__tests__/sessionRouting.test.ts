...
import { getRecoverableSessionHref, resolveSessionHref } from "../lib/sessionRouting";
...
describe("resolveSessionHref", () => {
  it.each([
    [{ status: "draft" as const, jobId: undefined }, "/studio"],
    [{ status: "error" as const, jobId: undefined }, "/studio"],
    [{ status: "canceled" as const, jobId: undefined }, "/studio"],
...
describe("getRecoverableSessionHref", () => {
  it.each([
    [{ status: "queued" as const, jobId: "job-123" }, "/jobs/job-123"],
    [{ status: "running" as const, jobId: "job/123?foo=bar" }, "/jobs/job%2F123%3Ffoo%3Dbar"],
    [{ status: "done" as const, jobId: "job-456" }, undefined],
    [{ status: "queued" as const, jobId: undefined }, undefined],
  ])("maps %o to %s", (input, expected) => {
    expect(getRecoverableSessionHref(input)).toBe(expected);
  });
});
