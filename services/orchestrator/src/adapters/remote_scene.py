...
from .base import AdapterCanceled, AdapterResult, BaseAdapter
...
class RemoteSceneAdapter(BaseAdapter):
    provider_id = "diffusion360_relay"
    modality = "scene"
    max_concurrency = 1
    ...
    def run(
        ...
        scene = _module(uir, "scene")
        client = RelayClient()
        created = client.create_task(
            {
                "job_id": job_id,
                "kind": "scene",
                "input": {"prompt": scene.get("prompt", "")},
                "options": {
        ...
        while True:
            ...
            reporter.stage(status, _progress(state.get("progress")), "relay scene")
            _check_canceled(reporter)
            if status == "failed":
